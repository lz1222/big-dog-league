...
Round 12 录包中最长侧距缺测为13个真实点云帧，当前
`side_missing_confirm_frames=15`。该值只延后永久故障锁存：第1个缺测帧起
`desired_vx/desired_wz` 就立即归零，量测恢复后连续计数清零；达到第15帧仍
缺测才进入 `FAULT_STOP`。

紧凑迷宫第一弯后的直段不足 `turn_start_distance_m`。因此
`CORRIDOR_REACQUIRE` 现在用“侧向居中、Yaw对齐、前距高于
`front_emergency_distance_m`”完成路线递增，不再要求前距重新大于 `0.62m`；
前距达到 `0.43m` 或更低仍立即锁止，不能借重捕获条件继续前进。
...
- 目标开放侧不小于 `turn_open_distance_m`，当前首弯录包标定值为 `0.42m`。
- 目标斜前距离不小于约 `0.38m`。
...
严格开口条件连续确认后才锁存 `TURN_LEFT/RIGHT`。连续确认期间允许已越过
`0.42m` 的目标侧使用 `turn_open_hysteresis_m=0.06m` 保持开口锁存，以跨过
低挡板扫描相位造成的数厘米回落；跌破 `0.36m` 会立即清除锁存。正前、目标
斜前和对侧条件仍逐帧校验。转向过程中，目标侧可能重新看到身旁墙端，因此不再
要求它持续大于开口门限；此时仍要求正前和目标斜前不小于约 `0.38m`、对侧及
任何重新出现的目标侧回波不小于 `0.185m`。滞回只防止入口确认抖动，不降低
机身即时碰撞余量。
...
| `turn_open_hysteresis_m` | 开口严格确认后的退出滞回，不参与扩大即时安全空间 |
...
- 走廊左右侧距缺失不会被当作开放空间。
- 单帧侧距缺失先暂停，连续缺失才锁定故障。
- Round 12 的13帧连续侧距缺测全程保持零诊断速度，恢复后可继续；第15帧锁止。
- 连续短墙候选不能跨帧改写强确认距离，防止缓存棘轮式下降。
- 同一 `cloud_sequence` 重发不能推进转向开口确认。
- 目标开口侧可缺测，但对侧缺测绝不允许开始转弯。
- 严格开口锁存后的扫描相位回落使用退出滞回，跌破退出线会清除确认计数。
- 接近开口时只跟随转向对侧墙，开口侧跳变不改变居中诊断。
- 转向启动后目标侧墙端回波仍高于 `0.185m` 时不得误报封路。
- 短通道可在前距高于紧急线时完成重捕获，达到紧急线则锁止。
...
2026-08-01 Round 12 由操控员人工完成首个左弯，入口尺量为前距 `107cm`、
左前/左后 `19cm`、右前/右后 `21cm`；弯后尺量为前距 `28cm`、左前/左后
`18cm`、右前/右后 `20cm`。完整录包位于
`/home/unitree/maze_bags/maze_manual_full_20260801_230648`。使用本节配置从原始
PointCloud2 重新提取并离线送入 B2 后，状态依次经过
`WAIT_SENSOR -> CORRIDOR_FOLLOW -> CORNER_APPROACH -> TURN_LEFT ->`
`TURN_FINE_ALIGN -> CORRIDOR_REACQUIRE -> CORRIDOR_FOLLOW`，没有进入
`FAULT_STOP`。默认 `0.42m` 开口和3帧确认在 `front=0.564m` 时锁存左转；
精调结束误差约 `4.27deg`，随后路线推进为 `route=1/5`。录包剩余数据已开始
等待第二个左弯，因此最终状态为 `CORNER_APPROACH / waiting_for_turn_opening`。
这是对人工轨迹的离线策略验收，只证明首弯数据可被正确解释，不代表代码已自主
控制机械狗，也不代表五弯迷宫真机PASS。

2026-08-02 Round 13 前两次检查点正常，操控员随后连续执行两次点动，人工前距
依次为 `67cm` 和 `50cm`。最终四角净间隙仍为左 `18/19cm`、右 `21/20cm`，
但旧 B1 右距缓存降至 `0.184m`，B2 正确按当时输入锁止
`corner_side_clearance_unsafe`。录包位于
`/home/unitree/maze_bags/b2_round13_20260802_002728`，共 `18880` 帧原始点云。
分析确认低值由 `continued_projected` 短簇反复写回缓存造成，不是真实正侧墙。
修正后正式参数重放中右侧最小值为 `0.2027m`、安全线以下为0帧，B2保持
`CORNER_APPROACH / approaching_turn_start`；Round 12 回归仍推进到
`route=1/5`。Round 13 未执行实际转弯，仍标记为 **未完成/等待原位复测**。

2026-08-02 Round 15 在入口第一次点动后人工前距为 `85.5cm`，第二次点动后
前距为 `59cm`，说明当前遥控最短点动实际达到约 `26.5cm`，高于早期采用的
`18cm` 上界。第二次落点的 B1 前距为 `0.74~0.77m`，左侧开口、右侧墙和
矩形移动转向包络均已确认安全，但旧 `turn_start_distance_m=0.62m` 仍要求
继续前进，下一次点动存在越过触发区的风险。因此正式真机配置将启动距离提前到
`0.80m`，恢复距离同步提高到 `0.85m`，终端D点动提示改为保守的 `27cm`。
转向仍须通过开口、扫掠包络和3个真实点云帧确认。

加载标定配置后，Round 15 在线状态依次经过
`TURN_LEFT -> TURN_FINE_ALIGN -> CORRIDOR_REACQUIRE -> CORRIDOR_FOLLOW`，
在转弯进度约 `70.38deg`、误差约 `19.62deg` 时进入精调，在进度约
`85.37deg`、误差约 `4.63deg` 时进入重捕获，最终推进为 `route=1/5`，随后
因第二段很短进入 `CORNER_APPROACH / waiting_for_turn_opening`。共记录20812条
B2状态，`FAULT_STOP` 为0。完整录包位于
`/home/unitree/maze_bags/b2_round15_20260802_144706`。转后人工尺量为前距
`25cm`、左前/左后 `9/9cm`、右前/右后 `27/27cm`，机身与新走廊平行；由于
操控员确认转弯过程中发生碰板，本轮最终记为
**首弯策略状态机PASS/真机物理安全FAIL**。`route=1/5` 和 `4.63deg` 误差不能
抵消碰撞结果，该轮不得计入真机首弯通过次数，也不能扩展为完整迷宫真机PASS。
碰撞为左侧机身在转弯中段轻触内侧板。录包在约 `44.07deg` 时仍输出扫掠安全，
当时左距约 `0.393m`；旧动态检查只要求机身半宽加边界约 `0.185m`，没有覆盖
70cm长机身在中间角度的横扫。修正后，`TURN_LEFT/RIGHT` 粗转阶段要求内侧
距离不小于矩形外接半径加边界约 `0.413m`，进入 `TURN_FINE_ALIGN` 后才恢复
半宽检查。Round 15 同一路径会在更早的内侧距离下降时先输出零诊断速度，并按
持续危险帧进入 `turn_sweep_unsafe`，但该修正仍需下一轮真机无碰撞复测。
使用修正后的正式参数重放 Round 15 全部 B1 状态时，策略在转弯进度约
`4.52deg`、左距约 `0.362m` 时确认低于 `0.413m` 动态门限并进入
`FAULT_STOP / turn_sweep_unsafe`，早于实际中段碰板。该结果证明保护能拦截
旧轨迹，但也表示原有人工转弯半径不可继续沿用；下一轮必须先完成更大移动半径的
单弯标定，不能直接继续五弯路线。
