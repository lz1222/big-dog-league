...
标准 `body_x_*`、`body_y_*` 是机身中心矩形。若静止录包证明左右前腿在该
矩形外稳定产生点云，可临时启用 `front_leg_self_filter_enabled`，并用左右两块
独立的前腿矩形记录已测回波带。该选项默认 `false`，且不能用扩大整体机身矩形
替代：整体矩形会遮蔽机头正前方的真实挡板。状态 JSON 的
`map_statistics.leg_self_filtered_points` 单独记录该过滤量，便于静止验收审计。

启用前腿过滤只改变 Dry Run 的点云解释，不会发布速度，也不会解除
`execution_allowed=false`。必须以静止真机录包确认侧墙仍可见、正前方障碍仍可见
且当前足迹不再触发伪碰撞，才能把该标定作为后续安全模型候选。
...
局部图会区分完整墙段和只读短墙片段：短片段可将原始障碍点关联到有限墙证据，
用于回放因果审计；它不会被计入可靠转弯墙模型，也不会使任何候选轨迹放行。
Round15 bag 已在开发机按只读 SQLite 方式回放，产物位于
`/tmp/b2_1_a_round15_replay_summary.json`；该门禁通过不等同于物理第一弯通过。
