...
延续结果只保持最后一次强确认距离，不能改写距离锚点。短墙候选不能在启动、
点云断流或缓存清空后自行生成侧距，来源标记为 `continued_projected`。更近侧墙
必须由正侧直接点或具有完整纵向跨度的投影簇确认，避免前墙边缘跨帧逐步压低
缓存。
...
保留值只能保持最后一次强确认净空，更近的强确认量测仍立即生效；真实开口确认最多延迟
约 `8 / 14.7 = 0.54s`。该结果是录包离线验证，仍需原位静止复测，不能写成
动态迷宫 PASS。

2026-08-01 Round 12 首弯完整录包
`/home/unitree/maze_bags/maze_manual_full_20260801_230648` 显示，旧
`side_projection_x_min=0.45m` 会把位于 `x=0.45..0.60m` 的拐角边缘投影成
约 `0.14m` 的右侧假近障。整包候选矩阵将起始位置提高到 `0.65m` 后，首弯
`right < 0.185m` 的错误帧由 `261` 降为 `0`，转弯阶段右距中位数约
`0.252m`；转弯前直道侧距可见率为 `100%`，转弯段为 `87.1%`，最长连续
缺测为 `13` 个真实点云帧。因此当前配置采用 `0.65m`，并由 B2 在任何缺测
首帧立即输出零诊断速度。以上仍是录包离线回放结果，不代表自动运动或整场迷宫
PASS。

2026-08-02 Round 13 在人工前距 `50cm`、左前/左后 `18/19cm`、右前/右后
`21/20cm` 时，旧稳定器把斜前短簇连续写回右侧缓存，使 B1 从约 `0.24m`
跨数百帧逐步下降到 `0.184m`，B2 因此锁止
`corner_side_clearance_unsafe`。故障后静止51帧的低值全部来自
`continued_projected/held_projected`，原始强投影最小值实际为 `0.2027m`。
修正后短簇只延续强确认距离、不再更新缓存；同包流式重放 `8808` 帧点云后，
右侧最小值为 `0.2027m`，低于或等于 `0.185m` 的帧数为 `0`。Round 12 首弯
回归仍完成 `route=1/5`。两项都是离线回放，不可写成新增真机运动 PASS。
